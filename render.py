...
def do_camera(time, mouse):
    """
    time: float
    mouse: np.array shape (4,), iMouse (xyzw)
...
def calc_look_at_matrix(ro, ta, roll):
    """
    ro: np.array shape (3,), camera position
    ta: np.array shape (3,), camera target
    roll: float
    returns: (3,3) np.array camera matrix
    """
    ww = normalize(ta - ro)
...
def main_image(iTime, fragCoord, iResolution):
    ...
    iMouse = np.array([0.0, 0.0, 0.0, 0.0])  # [mouseX, mouseY, mouseDownX, mouseDownY]
    #ro, ta = do_camera(iTime, iMouse)
    dh = (0.666 / iResolution[1])
    #rd = normalize(camMat @ np.array([p[0], p[1], 1.5]))  # 1.5 = lens focal length
    ...
    camMat = calc_look_at_matrix(ro, ta, roll=0.0)
    #rd = normalize(camTar - camPos)
    ...
    for i in range(AA_SAMPLES):
        ...
        rd = normalize(camMat * np.array([p[0]+dxy[0], p[1]+dxy[1], 1.5]))
        pos = ro
        hit = False
        #t = intersect_sphere(ro, rd)
        for i in range(ITERATIONS):
            t = DIST_SCALE * sdf(pos, iTime)
            pos += t * rd
...
# Render 
def render_image(time, iResolution=(800, 600)):
    ...
    for y in range(iResolution[1]):
        for x in range(iResolution[0]):
            ...
            iTime = 1.0  # placeholder
            fragCoord = np.array([x, y])  # current pixel coordinate
            img[y, x] = main_image(time, fragCoord, np.array(iResolution))
...
def update_frame(frame):
    time = frame / 30.0  # Normalize time to control the speed of the rotation
    img = render_image(time, iResolution)
    ax.imshow(img)
    ax.axis('off')

#output_image = render_image(iResolution)
ani = animation.FuncAnimation(fig, update_frame, frames=300, interval=50)
